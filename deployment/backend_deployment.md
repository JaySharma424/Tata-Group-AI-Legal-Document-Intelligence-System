...
* **Database:** Managed PostgreSQL 15 (SQLAlchemy ORM)
* **Vector Engine:** Qdrant Vector Store (In-Memory / Persistent Storage)
* **AI Orchestration:** LangGraph State Graph & Gemini Multimodal LLM Cascade
...
4. Render will auto-detect `render.yaml` and provision:
   ...
   * `tata-ai-frontend` (Static Site)
5. Set `GEMINI_API_KEY` under Environment Variables in the Render Dashboard.
...
| **Runtime** | Python 3.12 |
| **Region** | Singapore (or closest to DB) |

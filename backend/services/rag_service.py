...
class RAGKnowledgeService:
    """Production-grade RAG service with locked-storage resilience

    and structured legal record parsing.
    """

    def __init__(self, storage_path: str = "./backend/storage/qdrant_db"):
        self.collection_name = "tata_legal_knowledge"

        # 1. Attempt Disk Connection; Fall back to :memory: if locked by another process
        ...
        try:
            ...
            if "AlreadyLocked" in str(e) or "already accessed" in str(e):
                print(
                    "\n⚠️ Qdrant disk storage is locked by a running process."
                )
                print(
                    "⚙️ Falling back to in-memory Qdrant instance for this session.\n"
                )
                self.qdrant = QdrantClient(location=":memory:")
            else:
                raise e

        # 2. Local Dense Embedding Transformer (384 Dimensions)
    ...
    def _ensure_collection_exists(self):
        """Creates vector collection if missing or seeds if empty."""
        try:
            collections = [
    ...
    def _parse_structured_policy_file(
        ...
        for idx, block in enumerate(blocks):
            ...
            jurisdiction = (
                jurisdiction_match.group(1).strip()
    ...
    def _seed_structured_policies(self):
            ...
            try:
                with open(self.csv_path, mode="r", encoding="utf-8") as file:
                    ...
                    for row in reader:
                        ...
                        policy_txt = row.get("policy_text") or row.get(
                            "trigger", ""
                        )
                        guidelines = row.get(
                            "handling_guidelines"
                    ...
                    policies.append(
                        {
                            ...
                            "payload": {
                                "ref": rec["ref_id"],
                                "text": f"[{rec['ref_id']}] {rec['title']} - {rec['guidance']}",
                                "title": rec["title"],
        ...
        if points:
            ...
            print(
                f"✅ RAG Engine: Embedded {len(points)} structured legal records into Qdrant."
            )

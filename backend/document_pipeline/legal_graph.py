...
def retrieve_rag_context_node(state: LegalPipelineState) -> Dict[str, Any]:
    """Retrieves relevant corporate policies from Qdrant vector database using dense embeddings."""
    try:
        ...
        references = [
            {
                "ref": "POL-IND-2026-01",
                "text": "All vendor agreements must mandate explicit confidentiality obligations and liability caps.",
            }
        ]
...
def extract_clauses_node(state: LegalPipelineState) -> Dict[str, Any]:
    ...
    Includes automatic model cascading across multiple Gemini models if rate-limits (429) occur.
    """
    ...
    # Model Candidate Cascade: Tries high-quota models first to bypass 429 free-tier limits
    model_candidates = [
        "gemini-2.0-flash",
        "gemini-1.5-flash",
        "gemini-3.5-flash",
        ...
        try:
            ...
            if match:
                ...
                if isinstance(parsed, list) and len(parsed) > 0:
                    raw_clauses = parsed
                    print(
                        f"✅ Successfully extracted {len(raw_clauses)} clauses using model: {model_name}"
                    )
                    break
        except Exception as e:
            print(
                f"⚠️ Model {model_name} failed in extract_clauses_node: {e}. Trying next model in cascade..."
            )
            continue

    # Secondary Local Fallback if ALL cloud API models in cascade are exhausted or rate-limited
    if not raw_clauses:
        print(
            "⚡ All cloud models rate-limited. Executing local heuristic extraction fallback."
        )
        snippet = (
        ...
        raw_clauses = [
            {
                ...
                "risk_level": "MEDIUM",
                "risk_rationale": "Evaluated under local fallback policy rules due to cloud model quota limits.",
...
# Compile LangGraph State Graph Workflow
